...
> **Fork notice:** This repository (`jdb130496/PyPcre`) diverges from upstream
> [ModelCloud/PyPcre](https://github.com/ModelCloud/PyPcre) in one key way:
> the build system has been migrated from `setuptools` to
> [Meson](https://mesonbuild.com/) (`meson.build` + `meson.options`). All
> Python API, flags, benchmarks, and C extension functionality are identical
> to upstream and kept in sync by merging from `ModelCloud/PyPcre`.
...
Always do this after merging upstream or changing `meson.build`:
...
> **Important:** Always nuke `_build` and reinstall from scratch rather than
> just reinstalling. The editable loader caches the meson invocation path, and
> a stale `_build` can cause `meson.pyz` vs `meson.cmd` conflicts on Windows
> resulting in `%1 is not a valid Win32 application` errors.

### Verify the build

```bash
python -c "import pcre_ext_c; print('escape' in dir(pcre_ext_c))"   # True
python -c "import pcre; print(pcre.__version__)"                      # 0.6.0
```
...
If `meson.build` conflicts during merge:
...
All credit for the Python API, C extension, benchmarks, and PCRE2 integration
goes to [ModelCloud/PyPcre](https://github.com/ModelCloud/PyPcre). This fork
only changes the build system.
